...
async def check_the_task(message : types.Message):
    if not os.path.isfile('tester.py'):
        ...
        import tester
        reload(tester)
        await bot.send_message(message.from_user.id, f'отправлено задание с номером: {tester.NUMBER}.', reply_markup=ReplyKeyboardRemove())
        ...
        if not is_on:
            ...
            if tests.answer:
                await bot.send_message(message.from_user.id, 'Поздравляем! Все тесты пройдены✅')
            else:
                await bot.send_message(message.from_user.id, f'Тест {tests.counter + 1} не пройден!❌')
        os.remove("tester.py")
